...
const createCalcFee = async (api, version, multiplier) => {
...
const getExtrinsicBaseWeight = function (api, registry, metadata, version) {
  if (!( api.consts.system.extrinsicBaseWeight || api.consts.system.blockWeights.perClass.normal.baseExtrinsic)) {
    throw new InternalServerError("`extrinsicBaseWeight` is not defined");
  }
  ...
  if ( specName !== api.runtimeVersion.specName.toString() || specVersion !== api.runtimeVersion.specVersion.toNumber() ) {
    const decorated = expandMetadata(registry, metadata);
    extrinsicBaseWeight =
      decorated.consts.system?.extrinsicBaseWeight ||
      (decorated.consts.system?.blockWeights).perClass?.normal?.baseExtrinsic;
  } else {
    extrinsicBaseWeight =
  ...
  return extrinsicBaseWeight.toBigInt();
};

module.exports = {
  createCalcFee,
  getExtrinsicBaseWeight,
};
