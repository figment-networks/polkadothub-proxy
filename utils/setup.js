...
const setupApiAtHeight = async (api, height) => {
  ...
  let metaDataAt;
  if (metadataCache[runtimeVersionAt.specVersion]) {
    metaDataAt = metadataCache[runtimeVersionAt.specVersion];
  } else {
    metaDataAt = await api.rpc.state.getMetadata(blockHash);
  }
